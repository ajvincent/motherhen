 ...
 */
export default async function whereIsMyProject(config, settings) {
    const targetDirectory = path.normalize(path.resolve(projectRoot, settings.relativePathToConfig, "..", config.targetDirectory));
    let mozconfigPath;
    if (config.isFirefox) {
        mozconfigPath = path.join(targetDirectory, "firefox", config.buildType + ".mozconfig");
    }
    else {
...
${mozconfigPath}
  `.trim() + "\n");
    if (await fileExists(mozconfigPath, false)) {
        ...
        const objectDirectory = path.normalize(path.resolve(integrationRepo, line[1]));
        console.log(`
