 ...
 */
export default
async function getEditableJSON(
  pathToFile: string
) : Promise<EditableJSONWithExists>
{
  let exists = await fileExists(pathToFile, false);
  let output : WritableConfigurationJSON | null = null;
  ...
  if (exists) {
    ...
    let pass = false;
    {
      const contentsAsJSON = JSON.parse(contentsAsString) as unknown;
...
) : unknownContents is WritableConfigurationJSON
{
  ...
  const contents = unknownContents as object;
