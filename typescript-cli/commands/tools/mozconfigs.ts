...
export default
async function installMozconfig(
  targetDirectory: string,
  config: Required<FirefoxSummary | MotherhenSummary>,
) : Promise<void>
{
  const mozconfigsDir = path.join(
    targetDirectory,
    "mozconfigs",
    config.isFirefox ? "firefox" : config.applicationDirectory
  );
  ...
  if (config.isFirefox) {
    ...
    contents += await fs.readFile(
      path.join(cleanroomPath, "branding.mozconfig"), { encoding: "utf-8" }
    );
    contents += await fs.readFile(
